...
docker -H unix://"$DOCKER_SOCKET" run --privileged --dns=127.0.0.1 --dns-search=mesos -v "$DOCKER_SOCKET":/var/run/docker.sock -v "$WORKDIR":"$WORKDIR" $@ arangodb/dcos-mini-cluster $NUM_SLAVES $HOST_IP $PORT_RANGE $WORKDIR
